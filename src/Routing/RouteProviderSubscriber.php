...
final class RouteProviderSubscriber implements EventSubscriberInterface {
  ...
  /**
   * @deprecated since version 2.1, to be removed in 3.0. Use
   *   {@see RouteNameGenerator::ROUTE_NAME_PREFIX} instead.
   */
  ...
  public function __construct(
    ...
    $this->fileLoader = new XmlFileLoader(
      new FileLocator(
        __DIR__ . '/../DependencyInjection/Resources/config/routing'
      )
    );
  ...
  public function onDynamicRouteEvent(RouteBuildEvent $event) {
    ...
    foreach ($this->resourceClassDirectories as $directory) {
      $routeCollection->addResource(
        new DirectoryResource($directory, '/\.php$/')
      );
    }
...
//      $resourceClass = $this->resourceClassResolver->getActualResourceClass($resourceClass);
      ...
      if (NULL === $resourceShortName) {
        throw new InvalidResourceException(
          sprintf('Resource %s has no short name defined.', $resourceClass)
        );
      }
...
$gg =0;
  ...
  private function addRoute(
    ...
    $path = trim(
      trim($resourceMetadata->getAttribute('route_prefix', 'api')),
      '/'
    );
