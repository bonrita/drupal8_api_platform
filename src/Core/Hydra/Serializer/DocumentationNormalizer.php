...
use Drupal\api_platform\Core\Metadata\Property\SubresourceMetadata;
...
final class DocumentationNormalizer implements NormalizerInterface {
  ...
  public function normalize($object, $format = NULL, array $context = []) {
    ...
    foreach ($object->getResourceNameCollection() as $resourceClass) {
      ...
      // @todo check if ApiEntity annotation is set on the $resourceClass and add the real entity class to the context.
      $context['entity_class'] = true;

      $resourceMetadata = $this->resourceMetadataFactory->create($resourceClass);
      $shortName = $resourceMetadata->getShortName();
      $prefixedShortName = $resourceMetadata->getIri() ?? "#$shortName";
      $this->populateEntrypointProperties($resourceClass, $resourceMetadata, $shortName, $prefixedShortName, $entrypointProperties, $context);
   ...
   */
  private function populateEntrypointProperties(string $resourceClass, ResourceMetadata $resourceMetadata, string $shortName, string $prefixedShortName, array &$entrypointProperties, array $context = [])
  {
    $hydraCollectionOperations = $this->getHydraOperations($resourceClass, $resourceMetadata, $prefixedShortName, true, $context);
   ...
   */
  private function getHydraOperations(string $resourceClass, ResourceMetadata $resourceMetadata, string $prefixedShortName, bool $collection, array $context = []): array
  {
  ...
  private function getClass(string $resourceClass, ResourceMetadata $resourceMetadata, string $shortName, string $prefixedShortName, array $context): array
  {
    $class = [
      ...
      'hydra:supportedOperation' => $this->getHydraOperations($resourceClass, $resourceMetadata, $prefixedShortName, false, $context),
    ];
  ...
  private function getHydraProperties(string $resourceClass, ResourceMetadata $resourceMetadata, string $shortName, string $prefixedShortName, array $context): array
  {
    ...
    foreach ($classes as $class) {
      $options = $context +  $this->getPropertyNameCollectionFactoryContext($resourceMetadata);

      foreach ($this->propertyNameCollectionFactory->create($class, $options) as $propertyName) {
        $propertyMetadata = $this->propertyMetadataFactory->create($class, $propertyName, $context);
  ...
  private function computeDoc(Documentation $object, array $classes): array
  {
    ...
    $doc['hydra:entrypoint'] = $this->urlGenerator->generate('api_platform.api_entrypoint');
